...
public class ClientImpl implements IClient{
	...
	public static final int BLOCK_SIZE_MB = 1;
	...
	public boolean getBlock(int blockNo) throws RemoteException{
		StringBuffer sb = null;
		if (debugWriter) {
			sb = new StringBuffer();
			sb.append("\n***********GET ***********");
			sb.append("\nGet Req for block : " + blockNo );
		}
		BlockLocationResponse res = null;
		try {
			byte[] resBytes = nameNodeStub.getBlockLocations(SerializeHelper.buildBlockLocationRequest(blockNo));
			res = BlockLocationResponse.parseFrom(resBytes);
			...
			BlockInfo blockInfo = res.getBlockInfo();
	...
	public boolean get(String fileName, boolean forRead) throws RemoteException{
		...
		try {
			byte[] resBytes = nameNodeStub.openFile(SerializeHelper.buildOpenFileReq(fileName, forRead));
	...
	private boolean sendCloseReq(String fileName, boolean trStatus) {
		try {
			...
			/*byte[] resBytes =*/ nameNodeStub.closeFile(SerializeHelper.buildCloseFileRequest(fileName));
			if(debugWriter) {
				synchronized (logWriter) {
	...
	private byte[] readBlock(int dataNodeNo, int blockNo) {
		...
		try {
			...
			byte[] resBytes = stub.readBlock(SerializeHelper.buildReadReq(blockNo));
	...
	public List<String> list(String dirName) throws RemoteException{
		...
		try {
			byte[] resBytes = nameNodeStub.list(SerializeHelper.buildListFileReq(dirName));
		...
		try {
			...
			if (file.exists()) {
				...
				/*if (fileSize < BLOCK_SIZE) {
					...
					numberOfBlocks = splitFile(new File(path, fileName), 1);
				//}	
			}
